    ...
    try:
        ...
        if today_date in df['trade_date'].values:
            ...
            if today_date in holiday_dates:
                print(f"今天({today.strftime('%Y-%m-%d')} 星期{weekday+1})是节假日，不是交易日")
                return False
            else:
                # 不在节假日列表中，可能是调休工作日或交易日历计算错误
                # 假设是交易日，但记录警告
                print(f"⚠️ 警告: 今天({today.strftime('%Y-%m-%d')} 星期{weekday+1})不在交易日历中，但也不在节假日列表中，假设为交易日（可能因调休工作日被错误排除）")
                return True
    except Exception as e:
        print(f"获取交易日历失败: {e}")
        # 如果获取失败，检查是否是周末
